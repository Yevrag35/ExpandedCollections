...
The calculated index ({0}) must be, by definition, also less than the array's total count ({1}).";
        ...
        [Theory]
        [MemberData(nameof(TheList))]
        public void CountsMatch(int[] numbers)
        {
            var ordered = new OrderedList<int>(numbers);
            Assert.Equal(numbers.Length, ordered.Count);
        }

        [Fact]
        public void TestDefaultConstructors()
        {
            var ordered = new OrderedList<ICloneable>();
            Assert.Equal(0, ordered.Capacity);
        ...
        [InlineData(15000)]
        [InlineData(-1500)]
        public void TestCapacityConstructors(int capacity)
        {
            if (capacity > -1)
            {
                var ordered = new OrderedList<string>(capacity);
                Assert.Equal(capacity, ordered.Capacity);
                var ordered2 = new OrderedList<string>(capacity, StringComparer.CurrentCultureIgnoreCase);
                Assert.Equal(capacity, ordered2.Capacity);
                Assert.Equal(StringComparer.CurrentCultureIgnoreCase, ordered2.Comparer);
            }
            else
            {
                Assert.ThrowsAny<ArgumentOutOfRangeException>(() =>
                {
                    new OrderedList<string>(capacity);
                });
            }
        }

        [Theory]
        [MemberData(nameof(GetNumbersByIndex), parameters: 2)]
        public void TestEnumerableConstructor(int[] numbers)
        {
            var lookup = numbers
            ...
            var ordered = new OrderedList<int>(numbers);
            Assert.All(lookup.Keys, (key) =>
            {
                Assert.Equal(lookup[key], ordered.Count(x => key.Equals(x)));
            });
        }

        [Theory]
        [MemberData(nameof(GetNumbersByIndex), parameters: 0)]
        public void ListIsOrdered(int[] numbers)
        {
            var ordered = new OrderedList<int>(numbers);
            ...
            {
                Assert.Equal(numbers[i], ordered[i]);
            }
        }

        [Theory]
        [MemberData(nameof(GetNumbersByIndex), parameters: 0)]
        public void ListIsReversed(int[] numbers)
        {
            var reversed = new OrderedList<int>(numbers, new ReverseNumberComparer());
            for (int i = numbers.Length - 1; i >= 0; i--)
            {
                Assert.Equal(numbers[i], reversed[reversed.Count - 1 - i]);
            }
        }
